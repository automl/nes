                    ...
                    for j, M in enumerate(args.Ms):
                        ...
                        for pool_name in args.methods:
                            if pool_name in ["nes_re", "nes_rs",
                                ...
                                std = all_pools.std(axis=0)[-1]
                                mean = np.mean(all_pools, axis=0)[-1]
                                ...
                                if plot_individual_lines:
                                    for run_idx, run_data in enumerate(all_pools):
                                        if metric == "loss" and severity == 0 and M == 10 and data_type == "test" and ens_attr == "evals": print(run_data[-1]) 
                                        sty = ["solid", "dotted", "dashed", "dashdot", (0, (1, 10))][run_idx]

                                        ax.plot(x, run_data, label=label + f" run_{run_idx+1}", color=colors[pool_name],
                                                linewidth=2, marker=markers[pool_name], linestyle=sty,
                                                markersize=7, markevery=1 if args.dataset=="tiny" else 2)

                                else:
                                    ax.plot(x, mean, label=label, color=colors[pool_name],
                                            linewidth=2, marker=markers[pool_name],
                                            markersize=7, ls='--', markevery=1 if args.dataset=="tiny" else 2)
