#!/bin/bash
#SBATCH -a 1-20
#SBATCH -c 4
#SBATCH -o ./cluster_logs/nes_re/%A-%a.o
#SBATCH -e ./cluster_logs/nes_re/%A-%a.e
#SBATCH --gres=gpu:1  # reserves GPUs
#SBATCH -J imn-nes-re # sets the job name. If not specified, the file name will be used as job name

# Info
echo "Workingdir: $PWD"
echo "Started at $(date)"
echo "Running job $SLURM_JOB_NAME using $SLURM_JOB_CPUS_PER_NODE cpus per node with given JID $SLURM_JOB_ID on queue $SLURM_JOB_PARTITION, on gpu $SLURMD_NODENAME"

# Activate virtual environment
source venv/bin/activate

# Arrayjob
PYTHONPATH=$PWD python nes/optimizers/scripts/run_nes_re.py --array_id \
$SLURM_ARRAY_TASK_ID --total_num_workers=20 \
--num_iterations 200 --batch_size 128 --num_epochs 100 --population_size 50 --sample_size 10 \
--n_layers 8 --init_channels 36 --grad_clip --lr 0.1 --scheduler cosine \
--nic_name eth0 --working_directory experiments/tiny/baselearners/nes_re \
--global_seed $1 --scheme nes_re --dataset tiny

# Done
echo "DONE"
echo "Finished at $(date)"
