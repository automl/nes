#!/bin/bash
#SBATCH -a 1-6 #0-100
#SBATCH -o ./cluster_logs/deepens_darts/%A-%a.o
#SBATCH -e ./cluster_logs/deepens_darts/%A-%a.e
#SBATCH --gres=gpu:1  # reserves GPUs
#SBATCH -J anch_hyper # sets the job name. If not specified, the file name will be used as job name

# Info
echo "Workingdir: $PWD"
echo "Started at $(date)"
echo "Running job $SLURM_JOB_NAME using $SLURM_JOB_CPUS_PER_NODE cpus per node with given JID $SLURM_JOB_ID on queue $SLURM_JOB_PARTITION"

# Activate virtual environment
source activate python37

# Arrayjob
PYTHONPATH=$PWD python nes/optimizers/scripts/train_deepens_baselearner.py \
--seed_id $SLURM_ARRAY_TASK_ID --arch_id $SLURM_ARRAY_TASK_ID \
--working_directory "experiments-anchor/c10" \
--dataset cifar10 --num_epochs 100 --scheme deepens_darts \
--n_layers 8 --init_channels 16 --scheduler cosine \
--train_darts --global_seed 1 --anchor \
--lr 0.025 --wd 0.0 --anch_coeff 0.$SLURM_ARRAY_TASK_ID

# Done
echo "DONE"
echo "Finished at $(date)"
